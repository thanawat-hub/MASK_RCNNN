...
Original file is located at
    https://colab.research.google.com/drive/1YtIZkzb2CpPU2sR2c0QtPhtLs-whfyHf
"""

import os #ปัจจุบัน ก็ต้องมารันที่ตรงนี้อนยุ่แล้ว
pwd_current_leaf_script = os.getcwd() #C:\Users\user01\Desktop\mask_rcnn_find_area_pixel\area_leaf\samples\leaf_script
...
# ROOT_DIR = os.path.abspath("../../") #C:\Users\user01\Desktop\mask_rcnn_find_area_pixel\area_leaf\
...
# MODEL_DIR = r'C:\Users\user01\Desktop\mask_rcnn_find_area_pixel\area_leaf\logs\leaf20211206T0903' ## windows will is \  this com at nectec

MODEL_DIR = r'D:\________mask_rcnn________2\find_area_pixel_leaf_\logs\leaf20211206T0903' #this my com
# C:\Users\user01\Desktop\mask_rcnn_find_area_pixel\area_leaf\\logs\leaf20211206T0903
...
weights_path =  MODEL_DIR + '\mask_rcnn_leaf_0013.h5' ##r'/content/drive/MyDrive/_Mask_RCNN_more2class/IDV_MaskRCNN_Image_Segmentation-master/IDV_MaskRCNN_Image_Segmentation-master/logs/leaf20211203T0538/mask_rcnn_leaf_0013.h5' #
...
"""#ไม่ได้เอา ดึง mask instance มา"""
...
#ถ่ายภาพด้านล่าง side view
count_roi = 1
# bottom_count_roi = 0

roi_count_all = 1

"""# รับค่าตัวแปร ชื่อไฟล์มาจาก node-red"""

stdIN_filename_str_trimed = str(sys.stdin.readline()).strip()

# MODEL_DIR = r'C:\Users\user01\Desktop\mask_rcnn_find_area_pixel\area_leaf\logs\leaf20211206T0903'
# path_ = r"C:\Users\user01\Desktop\mask_rcnn_find_area_pixel\area_leaf\samples\leaf_script\new_add_dataset_more\test_image" # \
path_ = r"D:\________mask_rcnn________2\find_area_pixel_leaf_\samples\leaf_script\new_add_dataset_more\test_image"
path_name = path_+ '\\' + stdIN_filename_str_trimed #'3.jpg' # stdIN_filename
# print(path_name)

file_name_cant_detect = [] #เก็ฐชื่อไฟล์ที่ detectไม่ได้
...
#ติด ไว้ อาจต้องเทรดใหม่ แล้วตอนนี้ไปทำ th แบบปกติ

if r1['rois'].shape[0] != roi_count_all :
  print("model detect wrong cause this image have more\less {} roi \n   please change weight model this model can't find roi in right position with this image".format(roi_count_all))
  file_name_cant_detect.append(stdIN_filename_str_trimed)
else: #do the same 
  #visualize.display_instances(image_new2rgb, r1['rois'], r1['masks'], r1['class_ids'], r1['scores'], ax=ax, title="Predictions1")
  ...
  sort_roi = np.zeros(( roi_count_all , r1['rois'].shape[1])) # {roi_count_all} roi กับ 4 ตัวแปร x,y
  ...
  #เลือก แค่1 roi บน ที่เรียงค่า Y มาแล้ว เพราะเรารู็ว่า y อันแรก จะน้อยสุด คืออยู่ด้านบน จึงใช้ sort เลย
  # ได้ค่าของภาพด้านบน เพราะเรารู้ว่าด้านบนมีแค่ภาพเดียว ก้เลยเอาแค่ตัวแรกจากทั้งหมด
  sorted_y_from_XY_center_zip_list_up = sorted_y_from_XY_center_zip_list[:count_roi] # เอาตัวที่อยุ่ข้างบนก่อน
  ...
  #เลือก แค่2 roi ล่าง ที่เรียงค่า Y มาแล้ว เพราะเรารู็ว่า y 2 อันท้าย จะมากสุด 
  sorted_y_from_XY_center_zip_list_down = sorted_y_from_XY_center_zip_list[count_roi:] #รุ้ว่าข้างบนกี่ ก้เอาตั้งแต่นั้น
  ...
  for i in range(0,roi_count_all):
    for j in range(0,roi_count_all):                                                              # list_condition ก็ให้ตัว sort_roi ตัวนั้น คือค่าที่ตรงเงื่อนไข
  ...
  # ret, mask_instance_re_binary_th_ = cv2.threshold(mask_instance_re_binary, 5, 255, cv2.THRESH_BINARY)  
  # cv2_imshow(mask_instance_re_binary_th_)
  ret, mask_instance_re_binary_th = cv2.threshold(mask_instance_re_binary, 0, 255, cv2.THRESH_BINARY+cv2.THRESH_OTSU) 
  # cv2_imshow(mask_instance_re_binary_th)
  ...
  # cv2_imshow(input_image_copy_for_apply_mask)
  ...
  # cv2_imshow(newImage_apply_rm_outliers_rgb)                                                                                                                                                                                          #
  
  #== นำที่ตัดพื้นหลังไปทำต่อ
  apply_cv_rm_outliers_lab = cv2.cvtColor(input_image_copy_for_apply_mask, cv2.COLOR_BGR2LAB)
#

  count = 0 

  #name image
  # print('image name ',file_image_test.split('.')[0])
  # keep area each roi in 1 image
  list_area_each_roi = []
  ...
  for i in range(roi_count_all):#(r1['rois'].shape[0]):
    ...
    rois_apply_for_hsv = image_new[refPt[0][1]:refPt[1][1], refPt[0][0]:refPt[1][0]].copy() 

    # hsv ดึง s ออกมา
    hsv = cv2.cvtColor(rois_apply_for_hsv , cv2.COLOR_BGR2HSV)
    # cv2_imshow(s)
    blur_hsv = cv2.GaussianBlur(hsv,(5,5),0) #hsv
    h, s_blur, v = cv2.split(blur_hsv)

    ret,thresh_s_blured_at_hsv = cv2.threshold(s_blur,0,255,cv2.THRESH_BINARY+cv2.THRESH_OTSU)
    # cv2_imshow(thresh_s_blured_at_hsv)
  ...
  #save in directory
    ...
    cv2.imwrite(os.path.join(Path_keep_file_each_mask_lab , file_name_each_roi_mask_lab),  maskAll_test)
    count = count + 1 # end count for find and save
  ...
  # for i in range(len([name for name in os.listdir(DIR) if os.path.endswith(os.path.join(DIR, name))])):
  
  for i in range(len([name for name in os.listdir(DIR) if name.endswith('.jpg')])):
    roi_look_like_binary = cv2.imread(os.path.join(Path_keep_file_each_mask_lab , file_image_test.split('.')[0] + '_roi_'+ str(count+1)+ '.jpg'))
    ...
    # mask = cv2.inRange(hsv, lower_, upper_)# down shape to binary # just hsv rgb lab       
                                #
    roi_binary = cv2.cvtColor(roi_look_like_binary, cv2.COLOR_BGR2GRAY)                        
    ret, mask = cv2.threshold(roi_binary, 0, 255, cv2.THRESH_BINARY+cv2.THRESH_OTSU) 

    # cv2_imshow(mask)                                                                                                                                                                                          #
    or_maskLab_and_maskS = cv2.bitwise_or(thresh_s_blured_at_hsv,mask) #ที่เครื่อง nectec ทำได้ 
    # cv2_imshow(or_maskLab_and_maskS)
    ...
    contours1, hierarchy1  = cv2.findContours(or_maskLab_and_maskS, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)
    ...
    if len(contours1) <1:
      ...
      for c in range(0,len(contours1)):
        ...
        if mask.shape[0] * mask.shape[1] <= 250000 :# 500*500  คือถ้าภาพmask shape น้อยกว่า 500*500 ก็ให้ปริ้นค่า 
          ...
          if m['m00'] > 3600: #60x60
              list_area_each_roi.append(m['m00'])
              # print("area of roi_"+ str(count+1) +" = "+ str(m['m00']))
              print(m['m00'])
          else :
    ...
    count = count + 1 # count file in folder

    # # เขียนในแต่ละ file ของตัวเอง ถ้าไม่เปลี่ยน path
    # header = ['name', 'area_of_each_roi']

    # list_image_area = [file_image_test.split('.')[0], list_area_each_roi]
    ...
    #     # write the data
    #     writer.writerow(list_image_area)
    #     # จะเขียน ณ path ที่เราอยู่ ก็คือในโฟลเดอร์ ภาพ mask lab ในแต่ละอัน


# #เปลี่ยน path แต่ต้องออกมาหลังจากวนอ่าน ภาพทั้งหมด ที่ลงท้ายด้วย .jpg ก่อน 
# os.chdir('/content/drive/My Drive/find_area_pixel/samples/leaf_script/mask_image_from_test_input/for_csv_file') #fix for create csv file in folder "for_csv_fill"

# # list_image_area = list_image_area.append( (file_image_test.split('.')[0], list_area_each_roi) )
#   #if condition of in this line   
#     #  # if i == 2:
#      #   #   break
#       #  # else :
# with open(name_file, 'a', encoding='UTF8', newline='') as f: # a is append old file #if no have file will gen new file
#     # write the data
#     writer = csv.writer(f)
#     writer.writerow( (file_image_test.split('.')[0],list_area_each_roi) )
#     # จะเขียน ณ path ที่เราอยู่ ก็คือในโฟลเดอร์ for_csv_file
