...
import cv2#save input in fucntion display_instances too
...
def display_instances(image, boxes, masks, class_ids, class_names,
  ...
  #can do
    if name :
      if path_save:
        isWritten1 = plt.savefig(os.path.join(path_save , name))
        print('Image Predict instance segmentation is successfully saved as file in path')
 
        
def display_image_and_save_input_image(image, image_name, save_DIR):
    fig, ax = plt.subplots(2, figsize=(16,16))
    ax.axis('off')  # clear x-axis and y-axis
    ax.imshow(image.astype(np.uint8))
    plt.savefig(os.path.join(Path_keep_file_input, image_name))
